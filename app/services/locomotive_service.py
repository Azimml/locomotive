...
def _collect_inspection_dates(
    ...
    for column in selected_columns:
        ...
        dates[inspection_name] = (
            dt_value.to_pydatetime() if hasattr(dt_value, "to_pydatetime") else dt_value
        )
    ...
    if latest_repair_dates:
        for name, dt_value in latest_repair_dates.items():
            ...
            if hasattr(dt_value, "tz_convert"):
                dt_value = dt_value.tz_convert(None)
            dates[_canonical_repair_name(name)] = (
...
def _latest_repair_dates_by_loco(
    df_inspection: pd.DataFrame, df_types: pd.DataFrame
) -> dict:
...
def _repair_counts_by_year_for_loco(locomotive_id: int) -> dict:
    ...
    df_inspection = df_inspection.assign(year=df_inspection["last_updated_time"].dt.year)
    merged = df_inspection.merge(
        df_types, left_on="inspection_type_id", right_on="id", how="left"
    )

    counts = merged.groupby(["year", "name"]).size().reset_index(name="repair_count")

    result: dict = {}
    for year, group in counts.groupby("year"):
        group = group.sort_values(by="name")
        per_type = {
            (row["name"] if row["name"] is not None else "Unknown"): int(
                row["repair_count"]
            )
...
def _repair_counts_by_year_all() -> dict:
    ...
    counts = (
        merged.groupby(["locomotive_id", "year", "name"]).size().reset_index(name="repair_count")
    )
...
def list_locomotives() -> List[dict]:
    ...
    df_types = _load_table("inspection_inspectiontype", columns=["id", "name"])
    ...
    for loco_record in loco_records:
        ...
        latest_repair_dates = latest_repair_dates_by_loco.get(
        ...
        results.append(
            {
                ...
                "locomotive_type": (
                    model_record.get("locomotive_type") if model_record else None
                ),
...
def get_locomotive_info(locomotive_id: int | None = None, locomotive_name: str | None = None):
    ...
    if locomotive_id is not None:
        ...
        if not df_loco_exact.empty:
            ...
            df_loco = df_loco[df_loco["name"].astype(str).str.endswith(str(locomotive_name))]
...
def list_locomotive_models() -> List[dict]:
    ...
    counts = df_loco.groupby("locomotive_model_id").size().reset_index(name="locomotive_count")
...
def get_stats() -> dict:
    ...
    return {
        ...
        "state_counts": _records_without_nan(state_counts),
    }
...
def list_locomotive_types() -> List[dict]:
    ...
    merged = df_models.merge(
        df_loco, left_on="id", right_on="locomotive_model_id", how="left"
    )
    grouped = (
    ...
    return _records_without_nan(grouped)
...
def _inspection_type_counts(active_only: bool) -> list[dict]:
    ...
    merged["locomotive_count"] = merged["locomotive_count"].fillna(0).astype(int)
    merged = merged.drop(columns=["inspection_type_id"]).sort_values(by="id")

    return _records_without_nan(merged.rename(columns={"id": "inspection_type_id"}))


def list_inspection_counts(active_only: bool) -> list[dict]:
    return _inspection_type_counts(active_only=active_only)
...
def get_depo_info(depo_id: int) -> dict | None:
    ...
    df_branch = _load_table("organization_branch", columns=["name", "organization_id"])
    ...
    loco_type_counts = merged["locomotive_type"].value_counts(dropna=False).to_dict()
...
def get_depo_info_all() -> List[dict]:
    ...
    branch_names = {
        row["organization_id"]: row["name"] for row in _records_without_nan(df_branch)
    }
...
def list_active_repairs() -> List[dict]:
    ...
    df_loco = _load_table("locomotive_locomotive", columns=["id", "name", "state"])
...
def list_last_repairs_all() -> List[dict]:
    df_loco = _load_table("locomotive_locomotive", columns=["id", "name"])
    df_inspection = _load_table(
        "inspection_inspection",
        columns=["locomotive_id", "inspection_type_id", "last_updated_time"],
    )
...
def get_last_repair(locomotive_id: int | None = None, locomotive_name: str | None = None):
    if locomotive_id is None and not locomotive_name:
        raise ValueError("Provide locomotive_id or locomotive_name")
